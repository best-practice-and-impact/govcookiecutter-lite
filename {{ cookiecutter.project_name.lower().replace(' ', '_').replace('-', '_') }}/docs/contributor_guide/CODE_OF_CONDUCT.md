# Code of conduct for `{{ cookiecutter.repo_name.lower().replace(' ', '_').replace('-', '_') }}`
...
- "Project", we mean this {{ cookiecutter.repository_hosting_platform }} repository, `{{ cookiecutter.repo_name.lower().replace(' ', '_').replace('-', '_') }}` ;
